...
| `VLLM_PROMPT_USE_CHUNKED_ATTENTION` | Selects the chunked prompt-attention impl (`chunked_impl`), a flash-style path that tiles the key dimension with an fp32 online softmax. Intended for very long dense-MLA prefills where a single full-length attention op would OOM or exceed the FusedSDPA tile limit. MLA dense prefill only (no attention sinks or sliding window). | `false`                                          |
| `VLLM_PROMPT_CHUNK_SIZE`           | Key-dimension tile width (in tokens) for `chunked_impl`; smaller values lower peak activation memory at the cost of more iterations. `0` disables tiling (single full-length pass).                                    | `8192`                                           |
| `VLLM_FP32_ROPE`                   | Compute rotary position embeddings in fp32 (base `HPURotaryEmbedding` only) to avoid the bf16 phase-noise floor that grows with position. Improves byte-exact long-context fidelity; small prefill cost, zero KV cost. | `false`                                          |
