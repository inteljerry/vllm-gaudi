...
| `VLLM_FP32_ROPE`             | Computes rotary position embeddings in fp32 (base `HPURotaryEmbedding` only; scaled-rope subclasses ignore it). **On by default: bf16 RoPE returns wrong or empty answers above ~200K.** Measured on GLM-5.3 fp8 at 220,907 prompt tokens, bf16 scored 0/10 usable against 4/5 for fp32 (Fisher exact one-sided p = 0.0037), and the failure survives disabling n-gram speculation. `fused`/`1`/`true`/`on` routes through the fused Habana kernel in fp32; `manual` uses an unfused rotate_half fallback; `0`/`off` restores bf16. Costs about 20% decode tok/s at short context and about 3% at 128K-256K. | `fused` (unset or empty selects it) |
